...
class _CotizacionesFormularioScreenState extends State<CotizacionesFormularioScreen> {
  ...
  Future<void> _cargarCotizacionExistente() async {
    ...
    setState(() {
      ...
      _montoController.text = (cotizacion['presupuesto_monto'] ?? cotizacion['monto_total'] ?? 0.0).toString();
    ...
    try {
      final detalles = {
        'items': _items.where((item) => 
          ...
          item['cantidad'] > 0 && 
          (item['precio_unitario'] as double) > 0
        ).toList(),
        'incluye': _incluyeController.text.split(',').map((s) => s.trim()).where((s) => s.isNotEmpty).toList(),
        'no_incluye': _noIncluyeController.text.split(',').map((s) => s.trim()).where((s) => s.isNotEmpty).toList(),
        'especialidad': _especialidadController.text.trim(),
        'experiencia': _experienciaController.text.trim(),
      };

      final resultado = _isEditing
          ? await CotizacionesService.actualizarEstadoCotizacion(
              cotizacionId: widget.cotizacionId!,
              nuevoEstado: 'enviado',
              motivo: 'Actualizado desde formulario',
            )
          : await CotizacionesService.crearCotizacion(
              pescadorId: _pescadorIdController.text.trim(),
              capitanId: 'capitan-001', // ID del capitan logueado
              viajeId: _viajeIdController.text.trim(),
              titulo: _tituloController.text.trim(),
              descripcion: _descripcionController.text.trim(),
              montoTotal: _calcularTotal(),
              detalles: detalles,
              vigenciaDias: int.tryParse(_vigenciaDiasController.text) ?? 7,
            );

      if (resultado['success']) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(_isEditing ? 'Cotizacion actualizada correctamente' : 'Cotizacion creada correctamente'),
              ...
              _buildSectionCard(
                ...
                children: [
                  TextFormField(
                    ...
                    decoration: _buildInputDecoration('Titulo de la Cotizacion'),
                    validator: (value) {

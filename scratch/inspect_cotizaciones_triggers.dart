...
void main() async {
  ...
  try {
    // We can query pg_trigger through pg_catalog via REST if it's available, but usually it's not.
    // Let's try!
    final res = await supabase.from('pg_trigger').select('tgname');
    print('Triggers found: $res');
  } catch (e) {
    print('❌ Error querying pg_trigger: $e');
  }
}

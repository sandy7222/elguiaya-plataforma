...
void main() async {
  ...
  try {
    ...
    final List<dynamic> profiles = await client.from('profiles').select('user_id, rol, nombre');
    ...
    final List<dynamic> pescadores = await client.from('pescadores').select('*');
    ...
    final List<dynamic> guias = await client.from('guias').select('*');
    print('Total guias: ${guias.length}');
    for (var g in guias) {
      print('  ID: ${g['id']}, Nombre: ${g['nombre']}');
    }

    print('\n--- Querying notificaciones table count (unfiltered) ---');
    final List<dynamic> notifLegacy = await client.from('notificaciones').select('id');
    print('Legacy notifications count: ${notifLegacy.length}');

    print('\n--- Querying notificaciones_globales table count (unfiltered) ---');
    final List<dynamic> notifGlobal = await client.from('notificaciones_globales').select('id');

-- ============================================================
-- Variantes de venta (color/talle) — independientes de atributos
-- técnicos de ficha (tablas atributos / producto_atributos).
-- ============================================================

-- 1) Catálogo global de opciones (Color, Talle, ...)
...
CREATE TABLE IF NOT EXISTS public.producto_variantes (
  ...
  precio NUMERIC(12, 2), -- NULL = usar precio del producto padre
  imagen_url TEXT,
...
CREATE INDEX IF NOT EXISTS idx_producto_variantes_activo
  ON public.producto_variantes(producto_id, activo);

-- 3) Pedido: rastrear variante vendida
...
-- 6) Descontar stock de variante si aplica
...
BEGIN
  IF NEW.variante_id IS NOT NULL THEN
    ...
    -- Mantener stock del padre como suma de variantes activas
